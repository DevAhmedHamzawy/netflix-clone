        ...
        <div class="movies owl-carousel owl-theme">
            ...
            @foreach ($latest_movies as $latest_movie)
                <div class="movie text-white d-flex justify-content-center align-items-center">
                    ...
                    <div class="movie__bg"
                        style="background: linear-gradient(rgba(0,0,0, 0.6), rgba(0,0,0, 0.6)), url({{ $latest_movie->image_path }}) center/cover no-repeat;">
                    </div>
                                ...
                                <div class="movie__cta my-4">
                                    <a href="{{ route('movies.show', $latest_movie->id) }}"
                                        class="btn btn-primary text-capitalize mr-0 mr-md-2"><span
                                            class="far fa-play"></span> watch now</a>
                                    @auth
                                        <a href="#" class="btn btn-outline-light text-capitalize movie__fav-btn"><span
                                                class="far fa-heart movie__fav-icon movie-{{ $latest_movie->id }} {{ $latest_movie->is_favorite ? 'fw-900' : '' }}"
                                                data-movie-id="{{ $latest_movie->id }}"
                                                data-url="{{ route('movies.toggle_favorite', $latest_movie->id) }}"></span> add
                                            to favorite</a>
                                    @else
                                        <a href="{{ route('login') }}" class="btn btn-outline-light text-capitalize"><span
                                                class="far fa-heart"></span> add to favorite</a>
                                    @endauth
                ...
                <div class="row my-4">
                    <div class="col-12 d-flex justify-content-between">
                        ...
                        <a href="{{ route('movies.index', ['category_name' => $category->name]) }}"
                            class="align-self-center text-capitalize text-primary">see all</a>
                    </div>
                </div><!-- end of row -->
                            ...
                            <div class="movie__details text-white">
                                ...
                                <div class="d-flex movie__cta">
                                    <a href="{{ route('movies.show', $movie->id) }}"
                                        class="btn btn-primary text-capitalize flex-fill mr-2"><i class="fas fa-play"></i>
                                        watch now</a>
                                    @auth
                                        <i class="far fa-heart {{ $movie->is_favorite ? 'fw-900' : '' }} fa-1x align-self-center movie__fav-icon movie-{{ $movie->id }}"
                                            data-movie-id="{{ $movie->id }}"
                                            data-url="{{ route('movies.toggle_favorite', $movie->id) }}"></i>
                                    @else
                                        <a href="{{ route('login') }}" class="text-white align-self-center">
